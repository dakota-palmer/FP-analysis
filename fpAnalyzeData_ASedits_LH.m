...
figPath = 'Z:\Ally\GAD-VPFP DS Training\justraw\04to08\DP_newcode\GADLHFP\'; %location for output figures to be saved
...
%% PLOT AVERAGE TRACE OF EACH ANIMAL (across all days)
%initialize
    idDS=[];
    idNS=[];
    daysDS =[]; 
...
figureCount= figureCount+1; %iterate the figure count
for subj= 1:numel(subjects) %for each subject except #7 (#7 is GFP so don't want to average it- need to work on this, have and exclusion variable)
    
    for day=1:numel(subjDataAnalyzed.(subjectsAnalyzed{subj}));
%need to get ID for each time point, therefore just repeat it 
...
tracepumpall=vertcat(pumpDS,pumpNS); 

%% PLOTS of traces
%Stage 5
figure();
   a(1,1)=gramm('x',timeLocktracesall,'y', tracezblueall,'color',tracecuetype,'subset',tracestageall==5);
   a(1,1).stat_summary('type','sem','geom','area');
   a(1,1).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(1,1).set_title(' Average DS 465 nm Z Score-Stage 5')
   a(1,1).axe_property('YLim',[-2 3])
   a(1,1).set_color_options('map','brewer_dark')
 
   a(2,1)=gramm('x',timeLocktracesall,'y', tracezpurpleall,'color',tracecuetype,'subset',tracestageall==5);
   a(2,1).stat_summary('type','sem','geom','area');
   a(2,1).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(2,1).set_title(' Average DS 405 nm Z Score-Stage 5')
   a(2,1).axe_property('YLim',[-2 3])
   a(2,1).set_color_options('map','brewer_dark') 
%Stage 6
    a(1,2)=gramm('x',timeLocktracesall,'y', tracezblueall,'color',tracecuetype,'subset',tracestageall==6);
   a(1,2).stat_summary('type','sem','geom','area');
   a(1,2).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(1,2).set_title(' Average DS 465 nm Z Score-Stage 6')
   a(1,2).axe_property('YLim',[-2 3])
   a(1,2).set_color_options('map','brewer_dark')
 
   a(2,2)=gramm('x',timeLocktracesall,'y', tracezpurpleall,'color',tracecuetype,'subset',tracestageall==6);
   a(2,2).stat_summary('type','sem','geom','area');
   a(2,2).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(2,2).set_title(' Average DS 405 nm Z Score-Stage 6')
   a(2,2).axe_property('YLim',[-2 3])
   a(2,2).set_color_options('map','brewer_dark') 

   %Satge 7
   a(1,3)=gramm('x',timeLocktracesall,'y', tracezblueall,'color',tracecuetype,'subset',tracestageall==7);
   a(1,3).stat_summary('type','sem','geom','area');
   a(1,3).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(1,3).set_title(' Average DS 465 nm Z Score-Stage 7')
   a(1,3).axe_property('YLim',[-3 5])
   a(1,3).set_color_options('map','brewer_dark')

   a(2,3)=gramm('x',timeLocktracesall,'y', tracezpurpleall,'color',tracecuetype,'subset',tracestageall==7);
   a(2,3).stat_summary('type','sem','geom','area');
   a(2,3).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(2,3).set_title(' Average DS 405 nm Z Score-Stage 7')
   a(2,3).axe_property('YLim',[-3 5])
   a(2,3).set_color_options('map','brewer_dark') 
   
    %Satge 8
   a(1,4)=gramm('x',timeLocktracesall,'y', tracezblueall,'color',tracecuetype,'subset',tracestageall==8);
   a(1,4).stat_summary('type','sem','geom','area');
   a(1,4).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(1,4).set_title(' Average DS 465 nm Z Score-Stage 8')
   a(1,4).axe_property('YLim',[-3 5])
   a(1,4).set_color_options('map','brewer_dark')
   
   a(2,4)=gramm('x',timeLocktracesall,'y', tracezpurpleall,'color',tracecuetype,'subset',tracestageall==8);
   a(2,4).stat_summary('type','sem','geom','area');
   a(2,4).set_names('x','Time from Cue Onset (sec)','y','Z-Score')
   a(2,4).set_title(' Average DS 405 nm Z Score-Stage 8')
   a(2,4).axe_property('YLim',[-3 5])
   a(2,4).set_color_options('map','brewer_dark') 
   ...
   a.draw()
      ...
      %make figure full screen, save, and close this figure
set(gcf,'Position', get(0, 'Screensize')); %make the figure full screen before saving
saveas(gcf, strcat(figPath,'AvgTraces_SEM'),'fig');
%         close; %close 
...
% 
% for subj= 1:numel(subjects)
%   for day=1:numel(subjDataAnalyzed.(subjectsAnalyzed{subj}));%for each subject except #7 (#7 is GFP so don't want to average it- need to work on this, have and exclusion variable)
...
%     binstage=subjDataAnalyzed.(subjectsAnalyzed{subj})(day).trainStage;
%     binstage=repelem(binstage,length(indexbin))';
%     binidDS=vertcat(binidDS,binrepid');
%     bindaysDS=vertcat( bindaysDS,bindays);
%     binstageDS=vertcat(binstageDS,binstage);
...
%          binstageNS=vertcat(binstageNS,binstage);
...
% 
% %t-test
% %465 nm, paired t-test
% [criteriaindexDS,~]=find(binCriteriaStage5DS==1);
% [criteriaindexNS,~]=find(binCriteriaStage5NS==1);
% [binhyopothesis,binpvalue,binci,binstats] = ttest(binDSzSessionblueMean(criteriaindexDS),binNSzSessionblueMean(criteriaindexNS));
% criteriaindexall=vertcat(criteriaindexDS,criteriaindexNS);
% %ANOVA on 465nm DS and NS vs. 405nm DS and NS
% 
% %cue type
% cuetypeDS=repelem({'DS'},length(binDSzSessionblueMean))';
% cuetypeNS=repelem({'NS'},length(binNSzSessionblueMean))';
...
% bincriteriastage5alltype=[];
% binidallall=[];
% DScriteria=repelem(binCriteriaStage5DS,2);
% NScriteria=repelem(binCriteriaStage5NS,2);
% 
% 
% bincriteriastage5alltype=vertcat(bincriteriastage5allblue,bincriteriastage5allpurple);
% binidallall=repelem(binidall,2);
% 
%    %Overlay bar plots and scatter plot of average for cue type across all
%    %animals 
% figureCount=figureCount+1;
% figure();   
%    b(1,1)=gramm('x',bincuetype,'y', binzblueall,'color',bincuetype,'subset',binstageall==5);
%    b(1,1).stat_summary('type','sem','geom',{'bar','black_errorbar'});
%    b(1,1).set_names('x','Cue Type','y','Mean Z-Score')
%    b(1,1).set_title(' Average 465 nm Z Score-Stage 5')
%    b(1,1).axe_property('YLim',[-2 3])
%    b(1,1).set_color_options('map','brewer_pastel')
% 
%    b(2,1)=gramm('x',bincuetype,'y', binzpurpleall,'color',bincuetype,'subset',binstageall==5);
%    b(2,1).stat_summary('type','sem','geom',{'bar','black_errorbar'});
%    b(2,1).set_names('x','Cue Type','y','Mean Z-Score')
%    b(2,1).set_title(' Average 405 nm Z Score- Stage 5')
%    b(2,1).axe_property('YLim',[-2 3])
%    b(2,1).set_color_options('map','brewer_pastel')
%    b.draw()
%     
%    b(1,1).update('x',bincuetype,'y', binzblueall,'color',binidall,'subset',binstageall==6);
%    b(1,1).stat_summary('type','sem','geom','point');
%    b(1,1).set_names('x','Cue Type','y','Mean Z-Score')
%    b(1,1).set_title(' Average465 nm Z Score- Stage 6')
%    b(1,1).axe_property('YLim',[-2 3])
%    b(1,1).set_color_options('map','brewer_dark')
%     
% 
%    b(2,1).update('x',bincuetype,'y', binzpurpleall,'color',binidall,'subset',binstageall==6);
%    b(2,1).stat_summary('type','sem','geom','point');
%    b(2,1).set_names('x','Cue Type','y','Mean Z-Score')
%    b(2,1).set_title(' Average 405 nm Z Score- Stage 6')
%    b(2,1).axe_property('YLim',[-2 3])
%    b(2,1).set_color_options('map','brewer_dark')
%    b.draw()
% 
% 
%       %make figure full screen, save, and close this figure
% set(gcf,'Position', get(0, 'Screensize')); %make the figure full screen before saving
% saveas(gcf, strcat(figPath,'Bar_SEM'),'fig');
% %         close; %close 
% 
...
%     end
     ...
     stdFactor= 4; %multiplicative factor- how many stds away should we set our max & min color value? 
      % initialize
      topDSzblueA= [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurpleA= [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblueA = [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurpleA= [];
     
     %cond b
     topDSzblueB= [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurpleB= [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblueB = [];%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurpleB= [];
     ...
     if ~exist('currentSubj(1).DSzblueSessionMeanA')%only recording from later stages
     ...
     % create shared axis for stages 6, 7, 8
      ...
      elseif currentSubj(session).trainStage==7
         bottoms2= [bottomDSzblueD, bottomDSzblueE];
          tops2= [bottomDSzblueD, bottomDSzblueE];
      elseif currentSubj(session).trainStage==8
