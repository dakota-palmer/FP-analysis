...
%gcamp
excludedSubjs= {'rat20', 'rat16','rat10'}
%control
% excludedSubjs= {'rat8','rat9','rat11','rat12','rat13','rat14','rat15','rat17','rat19'} ; 
...
for subj= 1:numel(subjects)
    ...
    stagesToInclude=allStages %8:11; %allStages;%1:5;
...
% %% TIMELOCK TO DS
% 
% %----dp 2022-02-28   %instead of indexing with
%                 %cutTime(preEventTimeDS:postEventTimeDS), create a custom
%                 %time range with linspace() then interp() fp signal
...
%             preEventTimeDS = DSonset-preCueTime; %-preCueFrames; %earliest timepoint to examine is the shifted DS onset time - the # of frames we defined as periDSFrames (now this is equivalent to 20s before the shifted cue onset)
%             postEventTimeDS = DSonset+postCueTime; %postCueFrames; %latest timepoint to examine is the shifted DS onset time + the # of frames we defined as periDSFrames (now this is equivalent to 20s after the shifted cue onset)
% 
...
%                 %time range with linspace() then interp() fp signal
%                 %throughout this time range
%             baselineStart= DSonset-slideTime;
%             baselineEnd= DSonset;
%                 
...
% %             baselineStdBlue=nanstd(currentSubj(session).reblue((DSonset-slideTime):DSonset)); %baseline stdDev blue 10s prior to DS onset for boxA
% %             %purpleA
...
% %             baselineStdPurple=nanstd(currentSubj(session).repurple((DSonset-slideTime):DSonset)); %baseline stdDev purple 10s prior to DS onset for boxA
% 
...
%             %40hz time bins?
%             fs= 40;
%             tBins= round((postEventTimeDS-preEventTimeDS)*fs)+1; %plus one for t=0
%             
%             tRange=linspace(preEventTimeDS,postEventTimeDS,tBins)';  %time range over which to interp() fp signal (pre:post)
%             
% %             subjDataAnalyzed.(subjects{subj})(session).periDS.periDSwindow(:,:,cue)= interp1(preEventTimeDS,postEventTimeDS,tBins);
%             subjDataAnalyzed.(subjects{subj})(session).periDS.periDSwindow(:,:,cue)= tRange;
% 
...
% 
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSblue(:,:,cue)= periBlue;
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurple(:,:,cue)= periPurple;
%             
...
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSzblue(:,:,cue)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSzpurple(:,:,cue)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);
% 
%             
% %             %dff - *******Relies upon previous photobleaching/baseline
% %             section %interp has not been implemented here
...
%             subjDataAnalyzed.(subjects{subj})(session).periDS.baselineWindow(:,:,cue)= tRangeBaseline; %DSonset-slideTime:DSonset; 
%                 
%         
...
%             %of absolute time)
%             subjDataAnalyzed.(subjects{subj})(session).periDS.timeLock= tRange/fs;
%            
%             
%         end %end DS cue loop
%         
...
%----dp 2022-02-28   
%use raw timestamps for calculations and timelocking (independent of cutTime, no shifting of timestamps)
...
for subj= 1:numel(subjects) %for each subject
    ...
    %~~CHANGE THESE BASED ON TRIAL TYPE AND EVENT~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ (quick and dirty instead of writing fxn)
    %strings here will be evaluated with eval() to get the values
    %what cue type should baseline be timelocked to?
    eventBaseline= 'currentSubj(session).DS';

    %what event are you timelocking to?
    eventTimelock= 'currentSubj(session).DS';
    ...
    for session = 1:numel(currentSubj) %for each training session this subject completed              
        ...
        for trial=1:length(eval(eventTimelock)) %DS CUES %For each DS cue, conduct event-triggered analysis of data surrounding that cue's onset
            ...
            eventOnset= trialEvents(trial,1);
            baselineOnset= trialBaselineEvents(trial,1);

            %define the frames (datapoints) around each cue to analyze
            preEventTime = eventOnset-preCueTime; %-preCueFrames; %earliest timepoint to examine is the shifted DS onset time - the # of frames we defined as periDSFrames (now this is equivalent to 20s before the shifted cue onset)
            postEventTime = eventOnset+postCueTime; %postCueFrames; %latest timepoint to examine is the shifted DS onset time + the # of frames we defined as periDSFrames (now this is equivalent to 20s after the shifted cue onset)
            ...
            baselineStart= baselineOnset-slideTime;
            ...
            tBins= round((postEventTime-preEventTime)*fs)+1; %plus one for t=0
...
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeDS:postEventTimeDS);
            ...
            subjDataAnalyzed.(subjects{subj})(session).periDS.DS(trial) = eventOnset; %this way only included cues are saved
            subjDataAnalyzed.(subjects{subj})(session).periDS.periDSwindow(:,:,trial)= tRange;
            
            
            subjDataAnalyzed.(subjects{subj})(session).periDS.DSblue(:,:,trial)= periBlue;
            subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurple(:,:,trial)= periPurple;
            
                %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                %same interp approach for baseline                
            subjDataAnalyzed.(subjects{subj})(session).periDS.DSzblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
            subjDataAnalyzed.(subjects{subj})(session).periDS.DSzpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


            %save the time window for baseline calculations
            subjDataAnalyzed.(subjects{subj})(session).periDS.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 
                
        
            %lets save the baseline mean and std used for z score calc- so
            %that we can use this same baseline for other analyses
            subjDataAnalyzed.(subjects{subj})(session).periDS.baselineMeanBlue(1,trial)= baselineMeanBlue;
            subjDataAnalyzed.(subjects{subj})(session).periDS.baselineStdBlue(1,trial)= baselineStdBlue;
            subjDataAnalyzed.(subjects{subj})(session).periDS.baselineMeanPurple(1,trial)= baselineMeanPurple;
            subjDataAnalyzed.(subjects{subj})(session).periDS.baselineStdPurple(1,trial)= baselineStdPurple;
...
%instead of indexing with
                %cutTime(preEventTimeNS:postEventTimeNS), create a custom
                %time range with linspace() then interp() fp signal
                %throughout this time range
...
for subj= 1:numel(subjects) %for each subject
    ...
    %In this section, go cue-by-cue examining how fluorescence intensity changes in response to cue onset (either NS or NS)
    %Use an event-triggered sort of approach viewing data before and after cue onset where time 0 = cue onset time
    %Also, a sliding z-score will be calculated for each timepoint like in (Richard et al., 2018)- using data comprising 10s prior to that timepoint as a baseline  
    ...
    %strings here will be evaluated with eval() to get the values
    %what cue type should baseline be timelocked to?
    eventBaseline= 'currentSubj(session).NS';

    %what event are you timelocking to?
    eventTimelock= 'currentSubj(session).NS';
    ...
    for session = 1:numel(currentSubj) %for each training session this subject completed              
        ...
        cutTime= currentSubj(session).cutTime; %save this as an array, greatly speeNS things up because we have to go through each timestamp to find the closest one to the cues

        trialsSkipped= 0;  %counter to know how many cues were cut off/not analyzed (since those too close to the end will be chopped off- this shouldn't happen often though)
        
        for trial=1:length(eval(eventTimelock)) %NS CUES %For each NS cue, conduct event-triggered analysis of data surrounding that cue's onset
            ...
            %define the frames (datapoints) around each cue to analyze
            preEventTime = eventOnset-preCueTime; %-preCueFrames; %earliest timepoint to examine is the shifted NS onset time - the # of frames we defined as periNSFrames (now this is equivalent to 20s before the shifted cue onset)
            postEventTime = eventOnset+postCueTime; %postCueFrames; %latest timepoint to examine is the shifted NS onset time + the # of frames we defined as periNSFrames (now this is equivalent to 20s after the shifted cue onset)
            ...
            if postEventTime> length(currentSubj(session).cutTime)-slideTime %%if cue onset is too close to the end to extract following frames, skip this cue; if the latest timepoint to examine is greater than the length of our time axis minus slideTime (10s), then we won't be able to collect sufficient basline data within the 'slideTime' to calculate our sliding z score- so we will just exclude this cue
                disp(strcat(eventTimeLock, num2str(trial), ' too close to end, continuing'));
                trialsSkipped= trialsSkipped+1;  %iterate the counter for skipped NS cues
                continue %continue out of the loop and move onto the next NS cue
            end

            %instead of indexing with %cutTime(preEventTimeNS:postEventTimeNS), create a custom %time range with linspace() then interp() fp signal %throughout this time range
            baselineStart= baselineOnset-slideTime;
            baselineEnd= baselineOnset;
            ...
            %get mean and std during baseline for Z score
            baselineMeanBlue= nanmean(baselineBlue); %baseline mean blue 10s prior to NS onset
            baselineStdBlue= nanstd(baselineBlue); %baseline std blue 10s prior to NS onset 
            baselineMeanPurple= nanmean(baselinePurple); %baseline mean blue 10s prior to NS onset
            baselineStdPurple= nanstd(baselinePurple); %baseline std blue 10s prior to NS onset 
...
%             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeNS:postEventTimeNS);
            ...
            %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            %save all of the following data in the subjDataAnalyzed struct under the peri Event field
            subjDataAnalyzed.(subjects{subj})(session).periNS.NS(trial) = eventOnset; %this way only included cues are saved
            subjDataAnalyzed.(subjects{subj})(session).periNS.periNSwindow(:,:,trial)= tRange;
            
            
            subjDataAnalyzed.(subjects{subj})(session).periNS.NSblue(:,:,trial)= periBlue;
            subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurple(:,:,trial)= periPurple;
            
                %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                %same interp approach for baseline                
            subjDataAnalyzed.(subjects{subj})(session).periNS.NSzblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
            subjDataAnalyzed.(subjects{subj})(session).periNS.NSzpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


            %save the time window for baseline calculations
            subjDataAnalyzed.(subjects{subj})(session).periNS.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 
                
        
            %lets save the baseline mean and std used for z score calc- so
            %that we can use this same baseline for other analyses
            subjDataAnalyzed.(subjects{subj})(session).periNS.baselineMeanBlue(1,trial)= baselineMeanBlue;
            subjDataAnalyzed.(subjects{subj})(session).periNS.baselineStdBlue(1,trial)= baselineStdBlue;
            subjDataAnalyzed.(subjects{subj})(session).periNS.baselineMeanPurple(1,trial)= baselineMeanPurple;
            subjDataAnalyzed.(subjects{subj})(session).periNS.baselineStdPurple(1,trial)= baselineStdPurple;

            %save timeLock time axis- 'normalized' relative time from event (instead
            %of absolute time)
            subjDataAnalyzed.(subjects{subj})(session).periNS.timeLock= tRange/fs;
       %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if isnan(subjDataAnalyzed.(subjects{subj})(session).periNS.NS)
...
% % 
% %             %each entry in NS is a timestamp of the NS onset
% %             %~~~ FLAG: NSshifted here may be incorrectly getting events and
% %             %outcomes
% % %             NSonset = find(cutTime==currentSubj(session).NSshifted(cue,1));
% % %             NSonset = find(cutTime==currentSubj(session).NS(cue,1));
% %             NSonset= currentSubj(session).NS(cue,1);
% % 
% % 
% %             %define the frames (datapoints) around each cue to analyze
% %             preEventTimeNS = NSonset-preCueTime; %-preCueFrames; %earliest timepoint to examine is the shifted NS onset time - the # of frames we defined as periNSFrames (now this is equivalent to 20s before the shifted cue onset)
% %             postEventTimeNS = NSonset+postCueTime; %postCueFrames; %latest timepoint to examine is the shifted NS onset time + the # of frames we defined as periNSFrames (now this is equivalent to 20s after the shifted cue onset)
% % 
% %             if preEventTimeNS< 1 %if cue onset is too close to the beginning to extract preceding frames, skip this cue
% %                 disp(strcat('****NS cue ', num2str(cue), ' too close to beginning, continuing'));
% %                 NSskipped= NSskipped+1;
% %                 continue
% %             end
% % 
% %             if postEventTimeNS> length(currentSubj(session).cutTime)-slideTime %%if cue onset is too close to the end to extract following frames, skip this cue; if the latest timepoint to examine is greater than the length of our time axis minus slideTime (10s), then we won't be able to collect sufficient basline data within the 'slideTime' to calculate our sliding z score- so we will just exclude this cue
...
% %                 %time range with linspace() then interp() fp signal
% %                 %throughout this time range
% %             baselineStart= NSonset-slideTime;
% %             baselineEnd= NSonset;
% %                 
% %             fs= 40; %40hz time bins
% %             tBinsBaseline= round((baselineEnd-baselineStart)*fs);
...
% % 
% % %             
% % %             % Calculate average baseline mean&stdDev 10s prior to NS for z-score
% % %             %blueA
% % %             baselineMeanBlue=nanmean(currentSubj(session).reblue((NSonset-slideTime):NSonset)); %baseline mean blue 10s prior to NS onset for boxA
% % %             baselineStdBlue=nanstd(currentSubj(session).reblue((NSonset-slideTime):NSonset)); %baseline stdDev blue 10s prior to NS onset for boxA
% % %             %purpleA
% % %             baselineMeanPurple=nanmean(currentSubj(session).repurple((NSonset-slideTime):NSonset)); %baseline mean purple 10s prior to NS onset for boxA
% % %             baselineStdPurple=nanstd(currentSubj(session).repurple((NSonset-slideTime):NSonset)); %baseline stdDev purple 10s prior to NS onset for boxA
% % 
% %             %save all of the following data in the subjDataAnalyzed struct under the periNS field
% % 
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.NS(cue) = currentSubj(session).NS(cue); %this way only included cues are saved
% % 
% % 
...
% %             %40hz time bins?
% %             fs= 40;
% %             tBins= round((postEventTimeNS-preEventTimeNS)*fs)+1; %plus one for t=0
% %             
% %             tRange=linspace(preEventTimeNS,postEventTimeNS,tBins)';  %time range over which to interp() fp signal (pre:post)
% %             
% % %             subjDataAnalyzed.(subjects{subj})(session).periNS.periNSwindow(:,:,cue)= interp1(preEventTimeNS,postEventTimeNS,tBins);
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.periNSwindow(:,:,cue)= tRange;
% % 
% %             %interp() fp signals over our time range for this trial
...
% %             periPurple=  interp1(currentSubj(session).cutTime,currentSubj(session).repurple, tRange);
% % 
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.NSblue(:,:,cue)= periBlue;
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurple(:,:,cue)= periPurple;
% %             
% %                 %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
% %                 %same interp approach for baseline                
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.NSzblue(:,:,cue)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.NSzpurple(:,:,cue)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);
% % 
% %             
% % %             %dff - *******Relies upon previous photobleaching/baseline
% % %             section %interp has not been implemented here
...
% % %             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeNS:postEventTimeNS);
% % 
% % % %%~~~~~~~~~~~~~~~~exclude VP-VTA-FP08 session 21 NS cue 17 (came unplugged, big outlier)~~~~~!!!
% % %     
% % %             if subj == 1 && session==21 && cue == 17
% % %                subjDataAnalyzed.(subjects{subj})(session).periNS.NSblue(:,:,cue) = nan;
% % %                subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurple(:,:,cue) = nan;
% % %                subjDataAnalyzed.(subjects{subj})(session).periNS.NSzblue(:,:,cue) = nan;
% % %                subjDataAnalyzed.(subjects{subj})(session).periNS.NSzpurple(:,:,cue) = nan;
% % %             end
% %             
% %             %save the time window for baseline calculations
% %             subjDataAnalyzed.(subjects{subj})(session).periNS.baselineWindow(:,:,cue)= tRangeBaseline; %NSonset-slideTime:NSonset; 
% %                 
% %         
% %             %lets save the baseline mean and std used for z score calc- so
% %             %that we can use this same baseline for other analyses
...
% %         
% %    end %end session loop
% % end %end subject loop       clear cutTime  %this is cleared between sessions to prevent spillover
% %        
% %        cutTime= currentSubj(session).cutTime; %save this as an array, immensely speeNS things up because we have to go through each timestamp to find the closest one to the cues
% % 
% %   
% %       NSskipped= 0;  %counter to know how many cues were cut off/not analyzed (since those too close to the end will be chopped off- this shouldn't happen often though)
% % 
...
%        for cue = 1:numel(DSselected)
%             
%            if isnan(DSselected(cue)) %if there are no valid pe this session(e.g. on extinction days), make nan (otherwise might skip & fill in with 0s)
...
%            
%            elseif ~isnan(DSselected(cue)) %skip over trials where animal was in port at cue onset or did not make a PE during cue epoch
%                
%                
...
% %                 continue %continue out of the loop and move onto the next DS cue
% %                 %dp 1/5/22 found bug here where continuing out and leaving
% %                 %periDSpox fields empty... should pre fill them in case of
% %                 %continue
% %             end
% 
%             % Calculate average baseline mean&stdDev 10s prior to DS for z-score
...
%~~~TODO: DS trials where animal was in port at cue onset are NOT
%excluded!!!~~ 
%could simply exclude afterward.
...
for subj= 1:numel(subjects) %for each subject
    ...
    %what event are you timelocking to?
    eventTimelock= 'currentSubjAnalyzed(session).behavior.poxDS';
...
%         cutTime= currentSubj(session).cutTime; %save this as an array, greatly speeNS things up because we have to go through each timestamp to find the closest one to the cues
        ...
        for trial=1:length(eval(eventTimelock)) %NS CUES %For each NS cue, conduct event-triggered analysis of data surrounding that cue's onset
            ...
            tBins= round((postCueTime+preCueTime)*fs)+1; %plus one for t=0
...
%             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeNS:postEventTimeNS);
           ...
           if isnan(eventOnset)
                %~~~~~~if no event on this trial, make nan~~~~~~~~~~~update field names here based on events ~~~~~~~~~~~~
                tRange= [preCueTime:postCueTime]*fs;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSselected(trial) = nan;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.periEventWindow(:,:,trial)=nan(tBins,1);            
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxblue(:,:,trial)= nan(tBins,1);
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxpurple(:,:,trial)=nan(tBins,1);       
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxblue(:,:,trial)= nan(tBins,1); 
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxpurple(:,:,trial)=nan(tBins,1);

           elseif ~isnan(eventOnset)
                %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                %save all of the following data in the subjDataAnalyzed struct under the peri Event field            
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSselected(trial) = baselineOnset; %this way only included cues are saved
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.periEventWindow(:,:,trial)= tRange;


                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxblue(:,:,trial)= periBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxpurple(:,:,trial)= periPurple;

                    %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                    %same interp approach for baseline                
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


                %save the time window for baseline calculations
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 


                %lets save the baseline mean and std used for z score calc- so
                %that we can use this same baseline for other analyses
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.baselineMeanBlue(1,trial)= baselineMeanBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.baselineStdBlue(1,trial)= baselineStdBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.baselineMeanPurple(1,trial)= baselineMeanPurple;
                subjDataAnalyzed.(subjects{subj})(session).periDSpox.baselineStdPurple(1,trial)= baselineStdPurple;
    ...
    %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                %get the mean response to the NS for this session
            subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxblueMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxblue, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 

            subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxpurpleMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSpoxpurple, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 

            subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxblueMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxblue, 3);

            subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxpurpleMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSpox.DSzpoxpurple, 3);
   
% %     %TODO: matlab actually seems smart enough to autofill empty fields with nans where
% %     %appropriate, but rest of code logic assumes empty (e.g. checks for
% %     %isempty(periNS) in case of early stages without NS)
% %     
% %     %so if nan, replace fields with [];
%     if isnan(subjDataAnalyzed.(subjects{subj})(session).periNS.periNS)
%         subjDataAnalyzed.(subjects{subj})(session).periNS.NS = [];
%         subjDataAnalyzed.(subjects{subj})(session).periNS.periNSwindow= [];
%         
%         subjDataAnalyzed.(subjects{subj})(session).periNS.NSblue=[]; 
...
%~~~TODO: NS trials where animal was in port at cue onset are NOT
%excluded!!!~~ 
%could simply exclude afterward.
...
for subj= 1:numel(subjects) %for each subject
    ...
    %what event are you timelocking to?
    eventTimelock= 'currentSubjAnalyzed(session).behavior.poxNS';
...
%             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeNS:postEventTimeNS);
           ...
           if isnan(eventOnset)
                %~~~~~~if no event on this trial, make nan~~~~~~~~~~~update field names here based on events ~~~~~~~~~~~~
                tRange= [preCueTime:postCueTime]*fs;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSselected(trial) = nan;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.periEventWindow(:,:,trial)=nan(tBins,1);            
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxblue(:,:,trial)= nan(tBins,1);
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxpurple(:,:,trial)=nan(tBins,1);       
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSzpoxblue(:,:,trial)= nan(tBins,1); 
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSzpoxpurple(:,:,trial)=nan(tBins,1);

           elseif ~isnan(eventOnset)
                %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                %save all of the following data in the subjDataAnalyzed struct under the peri Event field            
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSselected(trial) = baselineOnset; %this way only included cues are saved
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.periEventWindow(:,:,trial)= tRange;


                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxblue(:,:,trial)= periBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxpurple(:,:,trial)= periPurple;

                    %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                    %same interp approach for baseline                
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSzpoxblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSzpoxpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


                %save the time window for baseline calculations
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 


                %lets save the baseline mean and std used for z score calc- so
                %that we can use this same baseline for other analyses
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.baselineMeanBlue(1,trial)= baselineMeanBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.baselineStdBlue(1,trial)= baselineStdBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.baselineMeanPurple(1,trial)= baselineMeanPurple;
                subjDataAnalyzed.(subjects{subj})(session).periNSpox.baselineStdPurple(1,trial)= baselineStdPurple;
    ...
    %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                %get the mean response to the NS for this session
            subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxblueMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxblue, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 

            subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxpurpleMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periNSpox.NSpoxpurple, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 
...
%     
%     %so if nan, replace fields with [];
  if ~isempty(subjDataAnalyzed.(subjects{subj})(session).periNS.NS)
    if isnan(subjDataAnalyzed.(subjects{subj})(session).periNS.NS)
        subjDataAnalyzed.(subjects{subj})(session).periNS.NS = [];
        subjDataAnalyzed.(subjects{subj})(session).periNS.periNSwindow= [];
...
for subj= 1:numel(subjects) %for each subject
    ...
    %what event are you timelocking to?
    eventTimelock= 'currentSubjAnalyzed(session).behavior.loxDS';
...
%             subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeDS:postEventTimeDS);
           ...
           if isnan(eventOnset)
                %~~~~~~if no event on this trial, make nan~~~~~~~~~~~update field names here based on events ~~~~~~~~~~~~
                tRange= [preCueTime:postCueTime]*fs;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSselected(trial) = nan;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.periEventWindow(:,:,trial)=nan(tBins,1);            
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxblue(:,:,trial)= nan(tBins,1);
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxpurple(:,:,trial)=nan(tBins,1);       
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSzloxblue(:,:,trial)= nan(tBins,1); 
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSzloxpurple(:,:,trial)=nan(tBins,1);

           elseif ~isnan(eventOnset)
                %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                %save all of the following data in the subjDataAnalyzed struct under the peri Event field            
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSselected(trial) = baselineOnset; %this way only included cues are saved
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.periEventWindow(:,:,trial)= tRange;


                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxblue(:,:,trial)= periBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxpurple(:,:,trial)= periPurple;

                    %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                    %same interp approach for baseline                
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSzloxblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSzloxpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


                %save the time window for baseline calculations
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 


                %lets save the baseline mean and std used for z score calc- so
                %that we can use this same baseline for other analyses
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.baselineMeanBlue(1,trial)= baselineMeanBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.baselineStdBlue(1,trial)= baselineStdBlue;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.baselineMeanPurple(1,trial)= baselineMeanPurple;
                subjDataAnalyzed.(subjects{subj})(session).periDSlox.baselineStdPurple(1,trial)= baselineStdPurple;
    ...
    %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                %get the mean response to the DS for this session
            subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxblueMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxblue, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 

            subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxpurpleMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periDSlox.DSloxpurple, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 
...
%     %TODO: matlab actually seems smart enough to autofill empty fields with nans where
% %     %appropriate, but rest of code logic assumes empty (e.g. checks for
% %     %isempty(periDS) in case of early stages without DS)
% %     
% %     %so if nan, replace fields with [];
%   if ~isempty(subjDataAnalyzed.(subjects{subj})(session).periDS.DS)
%     if isnan(subjDataAnalyzed.(subjects{subj})(session).periDS.DS)
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DS = [];
%         subjDataAnalyzed.(subjects{subj})(session).periDS.periDSwindow= [];
% 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSblue=[]; 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurple=[]; 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSzblue(1:periCueFrames+1,1,cue)= nan;
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSzpurple(1:periCueFrames+1,1,cue)= nan;
% 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSblueMean=[]; 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurpleMean=[]; 
% 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSzblueMean=[]; 
% 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSzpurpleMean= [];
% 
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSbluedff= [];
%         subjDataAnalyzed.(subjects{subj})(session).periDS.DSpurpledff= [];
%     end
%    end
%      
 %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%% TIMELOCK TO FIRST Lick AFTER NS
...
for subj= 1:numel(subjects) %for each subject
    ...
    %what event are you timelocking to?
    eventTimelock= 'currentSubjAnalyzed(session).behavior.loxNS';
...
%             subjDataAnalyzed.(subjects{subj})(session).periNS.NSpurpledff(:,:,cue)= subjDataAnalyzed.(subjects{subj})(session).photometry.purpledff(preEventTimeNS:postEventTimeNS);
           ...
           if isnan(eventOnset)
                %~~~~~~if no event on this trial, make nan~~~~~~~~~~~update field names here based on events ~~~~~~~~~~~~
                tRange= [preCueTime:postCueTime]*fs;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSselected(trial) = nan;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.periEventWindow(:,:,trial)=nan(tBins,1);            
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxblue(:,:,trial)= nan(tBins,1);
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxpurple(:,:,trial)=nan(tBins,1);       
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSzloxblue(:,:,trial)= nan(tBins,1); 
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSzloxpurple(:,:,trial)=nan(tBins,1);

           elseif ~isnan(eventOnset)
                %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                %save all of the following data in the subjDataAnalyzed struct under the peri Event field            
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSselected(trial) = baselineOnset; %this way only included cues are saved
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.periEventWindow(:,:,trial)= tRange;


                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxblue(:,:,trial)= periBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxpurple(:,:,trial)= periPurple;

                    %z score calculation: for each timestamp, subtract baselineMean from current photometry value and divide by baselineStd
                    %same interp approach for baseline                
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSzloxblue(:,:,trial)= ((periBlue-baselineMeanBlue))/(baselineStdBlue); 
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSzloxpurple(:,:,trial)= ((periPurple- baselineMeanPurple))/(baselineStdPurple);


                %save the time window for baseline calculations
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.baselineWindow(:,:,trial)= tRangeBaseline; %eventOnset-slideTime:eventOnset; 


                %lets save the baseline mean and std used for z score calc- so
                %that we can use this same baseline for other analyses
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.baselineMeanBlue(1,trial)= baselineMeanBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.baselineStdBlue(1,trial)= baselineStdBlue;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.baselineMeanPurple(1,trial)= baselineMeanPurple;
                subjDataAnalyzed.(subjects{subj})(session).periNSlox.baselineStdPurple(1,trial)= baselineStdPurple;
    ...
    %~~~~ SAVE DATA INTO STRUCT- CHANGE THESE FIELDNAMES BASED ON EVENTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                ...
                %get the mean response to the NS for this session
            subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxblueMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxblue, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 

            subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxpurpleMean = nanmean(subjDataAnalyzed.(subjects{subj})(session).periNSlox.NSloxpurple, 3); %avg across 3rd dimension (across each page) %this just gives us an average response to all cues 
...
%     subjTrial= cat(2, currentSubj.trainDay).'; %this is only training days for this subj
    subjTrial= 1:max([currentSubj.trainDay]); %assume start with day 1 and go until last train day (previous method can throw error if a train day is excluded (e.g. a subject missing day 1))
...
%          stdFactor= 4; %multiplicative factor- how many stds away do we want our color max & min?
% 
%          topDSzbluePump1= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump1mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%          topDSzbluePump2= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump2mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%          topDSzbluePump3= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump3mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          
%          
% %          topDSzpurple= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpurpleAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          bottomDSzbluePump1 = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump1mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%          bottomDSzbluePump2 = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump2mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%          bottomDSzbluePump3 = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzbluePump3mean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          
% %          bottomDSzpurple= -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpurpleAllTrials, 0, 2))));
% 
...
for subj= 1:numel(subjects)
    ...
    for thisStage= allStages %~~ Here we vectorize the field 'trainStage' to get the unique values easily %we'll loop through each unique stage
     ...
     topDSzblue= stdFactor*abs(nanmean((nanstd(DSblue, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurple= stdFactor*abs(nanmean((nanstd(DSpurple, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblue = -stdFactor*abs(nanmean((nanstd(DSblue, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurple= -stdFactor*abs(nanmean((nanstd(DSpurple, 0, 2))));
     ...
     %generate heatplots
        ...
        subplot(2, allStages(end), thisStage); title(strcat('DS 465nm Stage-',num2str(thisStage))); hold on;
        ...
        caxis([bottomAllDS,topAllDS]);
            ...
            %NS
            if thisStage>=5
                subplot(2, allStages(end), allStages(end)+thisStage); title(strcat('NS 465nm Stage-', num2str(thisStage))); hold on;

                imagesc(timeLock, 1:size(NSblue,2), NSblue');
                xlabel('time to cue (s)'); ylabel('trial');
                set(gca,'YDir','reverse'); %for some reason y axis is flipped using imagesc()
                ylim([1,size(NSblue,2)]); %this eliminates any extra white space
                caxis([bottomAllDS,topAllDS]);
            end
...
%        
       ...
       subjDataAnalyzed.(subjectsAnalyzed{subj})(session).effectSize.effectWindowDSzblueStd= nanstd(subjDataAnalyzed.(subjectsAnalyzed{subj})(session).periDS.DSzblue(effectWindow,1,:)); %this gives us std of response to each cue
       ...
       if isempty(currentSubj(session).periNS.NSzblue) %if there's no valid NS, there's no effect to look for
           ...
           subjDataAnalyzed.(subjectsAnalyzed{subj})(session).effectSize.effectWindowNSzblueStd= nanstd(subjDataAnalyzed.(subjectsAnalyzed{subj})(session).periNS.NSzblue(effectWindow,1,:)); %this gives us std of response to each cue
...
% 
%    for session = 1:numel(currentSubj) %for each training session this subject completed
%        artifactThreshold = nanstd(currentSubj(session).repurple)*2;
%        
...
%     dThreshold = nanstd(currentSubj(session).dPurple)*8;
...
%   scalingIntensity = 1; %nanmean(nanstd(currentSubj(1).periDS.DSzpurpleAllTrials));
...
%      
%      %cond D
%      topDSzblueD= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzblueSessionMeanD, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%      topDSzpurpleD= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpurpleSessionMeanD, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%      bottomDSzblueD = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzblueSessionMeanD, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
%      bottomDSzpurpleD= -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpurpleSessionMeanD, 0, 2))));
%      
...
% 
%              %cond A
%          topDSzblueA= stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueA, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          bottomDSzblueA = -stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueA, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%             %cond b
%          topDSzblueB= stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueB, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          bottomDSzblueB = -stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueB, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%             %cond c
%          topDSzblueC= stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueC, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          bottomDSzblueC = -stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueC, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%             %cond D
%          topDSzblueD= stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueD, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          bottomDSzblueD = -stdFactor*abs(nanmean((nanstd(allRats(1).grandMeanDSzblueD, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
% 
%          
...
if contains(experimentName, 'noArtifact')==true
    save(strcat(experimentName,'-', date, 'subjDataAnalyzed2-noArtifact.mat'), 'subjDataAnalyzed', '-v7.3'); %the second argument here is the variable being saved, the first is the filename %v7.3 .mat for files >2gb
elseif contains(experimentName, 'noArtifact')==false
    save(strcat(experimentName,'-', date, 'subjDataAnalyzed2.mat'), 'subjDataAnalyzed', '-v7.3'); %the second argument here is the variable being saved, the first is the filename %v7.3 .mat for files >2gb
end
