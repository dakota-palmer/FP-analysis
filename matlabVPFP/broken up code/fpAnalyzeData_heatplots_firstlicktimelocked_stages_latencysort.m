...
%                        continue %continue if no NS data
                    ...
                    currentSubj(1).DSloxrelpoxAllTrialsAlox=cat(2,currentSubj(1).DSloxrelpoxAllTrials,currentSubj(session).behavior.loxDSpoxRel(DSselected));
...
%                     currentSubj(1).DSloxAllTrialsA= cat(2,currentSubj(1).DSloxAllTrialsA,currentSubj(session).behavior.loxDS{DSselected});
                    ...
                    if ~isempty(currentSubj(session).periNSlox.NSselected)
                        ...
                        currentSubj(1).NSloxrelpoxAllTrialsAlox=cat(2,currentSubj(1).NSloxrelpoxAllTrials,currentSubj(session).behavior.loxNSpoxRel(NSselected));
...
%             allSubj(subj).NSpeLatencyAllTrialsA= currentSubj(1).NSpeLatencyAllTrialsA;
            ...
            %Condition B
                   if currentSubj(session).trainStage ==5
                        if sesCountB== 1 
                            currentSubj(1).DSzblueAllTrialsBlox= squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected));
                            currentSubj(1).DSzpurpleAllTrialsBlox= squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected));
                            ...
                            if ~isempty(currentSubj(session).periNSlox.NSselected) %if there's valid NS data
                                currentSubj(1).NSzblueAllTrialsBlox= squeeze(currentSubj(session).periNSlox.NSzblue(:,:,NSselected)); 
                                currentSubj(1).NSzpurpleAllTrialsBlox= squeeze(currentSubj(session).periNSlox.NSzpurple(:,:,NSselected));
...
%                                continue %continue if no NS data
                            ...
                            currentSubj(1).DSzblueAllTrialsBlox = cat(2, currentSubj.DSzblueAllTrialsBlox, (squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected)))); %concatenate- this contains z score response to DS from every DS (should have #columns= ~30 cues x #sessions)
                            currentSubj(1).DSzpurpleAllTrialsBlox = cat(2, currentSubj.DSzpurpleAllTrialsBlox, (squeeze(currentSubj(session).periDSlox.DSzpurple(:,:,DSselected)))); %concatenate- this contains z score response to DS from every DS (should have #columns= ~30 cues x #sessions)
                            ...
                            if ~isempty(currentSubj(session).periNS.NS)
                                currentSubj(1).NSzblueAllTrialsBlox = cat(2, currentSubj.NSzblueAllTrialsBlox, (squeeze(currentSubj(session).periNSlox.NSzblue(:,:,NSselected)))); 
                                currentSubj(1).NSzpurpleAllTrialsBlox = cat(2, currentSubj.NSzpurpleAllTrialsBlox, (squeeze(currentSubj(session).periNSlox.NSzpurple(:,:,NSselected)))); 
...
%                                 continue %continue if nos NS data
              ...
              %Condition C
               if currentSubj(session).trainStage >5
                    ...
                    if sesCountC== 1 
                        currentSubj(1).DSzblueAllTrialsClox= squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected));
                        currentSubj(1).DSzpurpleAllTrialsClox= squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected));
                        ...
                        if ~isempty(currentSubj(session).periNSlox.NSselected) %if there's valid NS data
                            currentSubj(1).NSzblueAllTrialsClox= squeeze(currentSubj(session).periNSlox.NSzblue(:,:,NSselected)); 
                            currentSubj(1).NSzpurpleAllTrialsClox= squeeze(currentSubj(session).periNSlox.NSzpurple(:,:,NSselected));
...
%                            continue %continue if no NS data
                        ...
                        currentSubj(1).DSzblueAllTrialsClox = cat(2, currentSubj.DSzblueAllTrialsClox, (squeeze(currentSubj(session).periDSlox.DSzblue(:,:,DSselected)))); %concatenate- this contains z score response to DS from every DS (should have #columns= ~30 cues x #sessions)
                        currentSubj(1).DSzpurpleAllTrialsClox = cat(2, currentSubj.DSzpurpleAllTrialsClox, (squeeze(currentSubj(session).periDSlox.DSzpurple(:,:,DSselected)))); %concatenate- this contains z score response to DS from every DS (should have #columns= ~30 cues x #sessions)
                        ...
                        if ~isempty(currentSubj(session).periNSlox.NSselected)
                            currentSubj(1).NSzblueAllTrialsClox = cat(2, currentSubj.NSzblueAllTrialsClox, (squeeze(currentSubj(session).periNSlox.NSzblue(:,:,NSselected)))); 
                            currentSubj(1).NSzpurpleAllTrialsClox = cat(2, currentSubj.NSzpurpleAllTrialsClox, (squeeze(currentSubj(session).periNSlox.NSzpurple(:,:,NSselected)))); 
...
%                             continue %continue if nos NS data
                    ...
                    subjSessC= cat(2, subjSessC, currentSubj(session).trainDay); %day count for y axis
                     ...
                     allSubj(subj).DSloxrelpoxAllTrialsBlox=currentSubj(1).DSloxrelpoxAllTrialsClox;
...
%    currentSubj(1).totalNScountA= 1:size(currentSubj(1).NSzblueAllTrialsA,1); 
    ...
    currentSubj(1).totalNScountF= 1:size(currentSubj(1).NSzblueAllTrialsF,1);
     ...
     topDSzblueE= stdFactor*abs(nanmean((std(currentSubj(1).DSzblueAllTrialsE, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurpleE= stdFactor*abs(nanmean((std(currentSubj(1).DSzpurpleAllTrialsE, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblueE = -stdFactor*abs(nanmean((std(currentSubj(1).DSzblueAllTrialsE, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     ...
     topDSzblueF= stdFactor*abs(nanmean((std(currentSubj(1).DSzblueAllTrialsF, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurpleF= stdFactor*abs(nanmean((std(currentSubj(1).DSzpurpleAllTrialsF, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblueF = -stdFactor*abs(nanmean((std(currentSubj(1).DSzblueAllTrialsF, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurpleF= -stdFactor*abs(nanmean((std(currentSubj(1).DSzpurpleAllTrialsF, 0, 2))));
      end
          %cond c
     if currentSubj(session).trainStage >5
