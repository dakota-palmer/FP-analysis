...
%     end
     ...
     topDSzbluepox= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxblueSessionMean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurplepox= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxpurpleSessionMean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzbluepox = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxblueSessionMean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurplepox= -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxpurpleSessionMean, 0, 2))));
    ...
    if ~isempty(currentSubj(1).NSzpoxblueSessionMean) %only run this if there's NS data
        topNSzpoxblue= stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxblueSessionMean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
        topNSzpoxpurple= stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxpurpleSessionMean, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

        bottomNSzpoxblue= -stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxblueSessionMean, 0, 2))));
        bottomNSzpoxpurple= -stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxpurpleSessionMean, 0, 2))));
...
currentSubj= subjDataAnalyzed.(subjectsAnalyzed{subj}); %use this for easy indexing into the current subject within the struct
    ...
    %TODO: split up yticks by session (this would show any clear differences between days)
     ...
     topDSzblue= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxblueAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     topDSzpurple= stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxpurpleAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

     bottomDSzblue = -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxblueAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
     bottomDSzpurple= -stdFactor*abs(nanmean((nanstd(currentSubj(1).DSzpoxpurpleAllTrials, 0, 2))));
    ...
    if ~isempty(currentSubj(1).NSzpoxblueAllTrials) %only run this if there's NS data
        topNSzblue= stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxblueAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor
        topNSzpurple= stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxpurpleAllTrials, 0, 2))));%std calculated for each cue (across all timestamps), then averaged, absolute valued, then multiplied by factor

        bottomNSzblue= -stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxblueAllTrials, 0, 2))));
        bottomNSzpurple= -stdFactor*abs(nanmean((nanstd(currentSubj(1).NSzpoxpurpleAllTrials, 0, 2))));
