...
figPath = 'Z:\Ally\Code\FP-analysis-variableReward\matlabVPFP\ASedits\broken up code\Figs\'; %location for output figures to be saved
...
allRats.grandDSzblue=nanmean(allRats.meanDSzblue(:,1:26,1:7),3)'; %(:,:,1:4),3)' % doing 1:4 in 3rd dmension because rat8 is a GFP animal but need to find more robust way to do this
allRats.grandDSzpurple=nanmean(allRats.meanDSzpurple(:,1:26,1:7),3)'; %1:4),3)'
allRats.grandNSzblue=nanmean(allRats.meanNSzblue(:,1:26,1:7),3)';%'; %1:4),3)'
allRats.grandNSzpurple=nanmean(allRats.meanNSzpurple(:,1:26,1:7),3)'; %,1:4),3)'
...
figureCount= figureCount+1; %iterate the figure count
for subj= 1:6 %for each subject except #7 (#7 is GFP so don't want to average it- need to work on this, have and exclusion variable)
...
%Stage 5-day 1- when NS is introduced
 ...
 %Day animal reached criteria 
   ...
   %make figure full screen, save, and close this figure
set(gcf,'Position', get(0, 'Screensize')); %make the figure full screen before saving
saveas(gcf, strcat(figPath,'AvgTraces_SEM_earlyandlate'),'fig');
%         close; %close 
...
%Stage 5
 ...
 %Satge 6
   ...
   %make figure full screen, save, and close this figure
set(gcf,'Position', get(0, 'Screensize')); %make the figure full screen before saving
saveas(gcf, strcat(figPath,'AvgTraces_SEM_stages'),'fig');
%         close; %close  
...
anovamatrix(1+length(anovamatrix(:,1)),1)=mean(anovacell{1}(NSidindex));%mean DS blue signal
anovamatrix(length(anovamatrix(:,2)),2)=mean(anovacell{2}(NSidindex));%mean DS purple signal
end
...
figureCount= figureCount+1; %iterate the figure count   
figure();   
   b=gramm('x',binsignalall,'y', binzall,'color',bincuetypeall,'subset',bincriteriastage5alltype==1);
...
b.draw()
   ...
   b.update('x',binsignalall,'y', binzall,'color',binidallall,'subset',bincriteriastage5alltype==1);
