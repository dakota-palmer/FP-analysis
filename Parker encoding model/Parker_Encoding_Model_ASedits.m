...
for subj=1:numel(subjects)
    ...
    gcamp_y_purple=data_to_input_GADVPFP.g_output(1).gcamp_raw.purple;
       ...
       %% Moving Z-score
% Here we are calculating the z-score 10 seconds before the DS    
% 
    z_gcamp_y_blue=[];
    z_gcamp_y_purple=[];
    ...
    for DS=1:size(DSTimes,2)
           ...
           %DSonset_b_y=gcamp_y_blue(DSonset_b);
            ...
            z_purple_std=std(gcamp_y_purple(DSonset-(tb*fs):DSonset));
             ...
             if DS==1
            z_gcamp_y_blue_first(:)=(gcamp_y_blue(1:DSonset-1)-z_blue_baseline)/z_blue_std;
            z_gcamp_y_purple_first(:)=(gcamp_y_purple(1:DSonset-1)-z_purple_baseline)/z_purple_std;
             end
             
            z_gcamp_y_blue_temp=[];
            z_gcamp_y_purple_temp=[];
            
            z_gcamp_y_blue_temp(:,:)=(gcamp_y_blue(DSonset: DSonset_b-1)-z_blue_baseline)/z_blue_std; fprintf('blue Z-scored \n')
            z_gcamp_y_purple_temp(:,:)=(gcamp_y_purple(DSonset: DSonset_b-1)-z_purple_baseline)/z_purple_std; fprintf('purple Z-scored \n')
            ...
            z_gcamp_y_blue_temp(:,:)=(gcamp_y_blue(DSonset:end)-z_blue_baseline)/z_blue_std; fprintf('blue Z-scored \n')
            z_gcamp_y_purple_temp(:,:)=(gcamp_y_purple(DSonset: end)-z_purple_baseline)/z_purple_std; fprintf('purple Z-scored \n')
